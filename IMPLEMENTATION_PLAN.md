...
- Exploratory QA against a live `codex app-server` also validated `thread/list`, `thread/loaded/list`, and `thread/read`; `thread/resume` on a freshly started thread currently returns a server-side `no rollout found` error, so the client now documents that only persisted rollouts are resumable and a deterministic happy-path resume fixture remains follow-up work.
